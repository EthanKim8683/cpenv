...
func renderTemplate(fs afero.Fs, tmpl string, src []byte, problem *problemv1.Problem) error {
	thread := &starlark.Thread{}

	problemValue, err := encodeProblem(thread, problem)
	if err != nil {
		return fmt.Errorf("render template %q: encode problem: %w", tmpl, err)
	}

	filesValue, err := execTemplate(thread, tmpl, src, problemValue)
	if err != nil {
		return fmt.Errorf("render template %q: exec: %w", tmpl, err)
	}

	files, err := decodeFiles(filesValue)
	if err != nil {
		return fmt.Errorf("render template %q: decode files: %w", tmpl, err)
	}

	if err := writeFiles(fs, files); err != nil {
		return fmt.Errorf("render template %q: write files: %w", tmpl, err)
	}
