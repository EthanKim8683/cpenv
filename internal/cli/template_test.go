...
func readTemplate(t *testing.T, name string) []byte {
	...
	src, err := os.ReadFile(filepath.Join("testdata", "template", name))
...
func TestRender(t *testing.T) {
	...
	t.Run("successful render", func(t *testing.T) {
		...
		require.NoError(t, renderTemplate(fs, tmpl, readTemplate(t, tmpl), problem))
	...
	t.Run("decode error", func(t *testing.T) {
		...
		err := renderTemplate(afero.NewMemMapFs(), tmpl, readTemplate(t, tmpl), nil)
		assert.Error(t, err)
		assert.ErrorContains(t, err, "decode files: ")
