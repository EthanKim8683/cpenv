...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/EthanKim8683/cpenv/internal/gen/status/v1"
	http "net/http"
	strings "strings"
)
...
func NewStatusServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) StatusServiceClient {
	...
	statusServiceMethods := v1.File_status_v1_status_service_proto.Services().ByName("StatusService").Methods()
	return &statusServiceClient{
		save: connect.NewClient[v1.SaveRequest, v1.SaveResponse](
			httpClient,
			baseURL+StatusServiceSaveProcedure,
			connect.WithSchema(statusServiceMethods.ByName("Save")),
...
func (c *statusServiceClient) Save(ctx context.Context, req *v1.SaveRequest) (*v1.SaveResponse, error) {
...
func NewStatusServiceHandler(svc StatusServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	statusServiceMethods := v1.File_status_v1_status_service_proto.Services().ByName("StatusService").Methods()
	statusServiceSaveHandler := connect.NewUnaryHandlerSimple(
		StatusServiceSaveProcedure,
		svc.Save,
		connect.WithSchema(statusServiceMethods.ByName("Save")),
		connect.WithIdempotency(connect.IdempotencyIdempotent),
